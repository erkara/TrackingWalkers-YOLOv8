                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_txt:
                            ...
                            with open(txt_path + '.txt', 'a') as f:
                                ...
                                # f.write(('%g ' * 10 + '\n') % (frame_idx + 1, id, bbox_left + 0.5 * bbox_w,  # just centroids
                                #                                bbox_top + 0.5 * bbox_h))

                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            c = int(cls)  # integer class
                            id = int(id)  # integer id
                            label = None if hide_labels else (f'id:{id} conf:{conf:.2f}' if hide_conf else \
                                                                  (
                                                                      f'{id} {conf:.2f}' if hide_class else f'{id}'))
                            annotator.box_label(bboxes, label, color=colors(c, True),txt_color=(0, 0, 255))
                            if save_crop:
